...
class Uploader
{
    ...
    private function saveRemote()
    {
        ...
        $imgUrl = $this->replaceBaiduDomain($imgUrl);
        ...
        ob_end_clean();
        preg_match("/[\/]([^\/]*)[\.]?[^\.\/]*$/", $imgUrl, $m);
        $str_info  = @unpack("C2chars", $img);
        $this->type_code = intval($str_info['chars1'].$str_info['chars2']);
    ...
    private function getFileExt()
    {
        ...
        if (!in_array($ext, ['.jpg', '.jpeg', '.png', '.gif']) && isset($this->type_code)) {
    ...
    private function getFullName()
    {
        ...
        //替换随机字符串
        $base10 = '0123456789';
        // $randNum = mt_rand(10000000, 99999999) . mt_rand(10000000, 99999999);
        // $randNum = rand(1, 10000000000) . rand(1, 10000000000);
        if (preg_match("/\{rand\:([\d]*)\}/i", $format, $matches)) {
            $rand_int_str = '';
            for ($i = 0; $i < $matches[1]; $i++) {
                $rand_int_str .= $base10[rand(0, 9)];
            }
            $format = preg_replace("/\{rand\:[\d]*\}/i", $rand_int_str, $format);
    ...
    public function replaceBaiduDomain($url)
    {
        $baidu_domain = [
        ...
        $patterns = array_keys($baidu_domain);
        $replacements = array_values($baidu_domain);
